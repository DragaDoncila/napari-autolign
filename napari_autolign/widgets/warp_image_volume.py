...
def make_image_warping(
    ...
    assert isinstance(moving_image, Image) or isinstance(moving_image, Labels), 'moving_image must be Image or Labels'
    assert isinstance(fixed_image, Image) or isinstance(fixed_image, Labels), 'fixed_image must be Image or Labels'
    ...
    if transform_type == 'Deformable':
        _warp_image_volume(moving_image=moving_image.data,
